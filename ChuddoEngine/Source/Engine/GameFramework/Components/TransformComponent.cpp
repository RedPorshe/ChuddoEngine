...
CTransformComponent::CTransformComponent ( CObject * inOwner, const std::string & inDisplayName )
	...
	{
	SetCollisionComponent ( this->AddSubObject<CBaseCollisionComponent> ( GetName () + "_Collision" ) );
	}
...
void CTransformComponent::UpdateTransformMatix ()
	...
	if (warnCount++ < 2) // Лимитируем предупреждения
		{
		LOG_WARN ( "need define UpdateTransformMatix() - TODO for rendering" );
		}
	}