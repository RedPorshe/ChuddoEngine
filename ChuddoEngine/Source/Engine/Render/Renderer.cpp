...
bool CRenderer::RenderScene ()
	...
	try
		...
		if (!EndFrame ( imageIndex ))
			...
			LogDebug ( "EndFrame failed, continuing..." );
			}
...
bool CRenderer::EndFrame ( uint32_t ImageIndex )
	...
	if (!m_SwapChainManager->Present ( renderFinishedSemaphore, ImageIndex ))
		{
		// Swap chain needs to be recreated - handled inside Present
		return false;
		}
...
bool CRenderer::RecreateSwapChainResources ()
	...
	// 1. Сначала очищаем старые command buffers
	...
	// 2. Пересоздаем swapchain (это делает SwapChainManager в AcquireNextImage/Present)
	// Но нам нужно убедиться, что RenderPassManager тоже обновился

	// 3. Пересоздаем framebuffers через RenderPassManager
	if (!m_RenderPassManager->CreateSwapChainResources ())
		{
		LogError ( "Failed to recreate framebuffers" );
		return false;
		}

	// 4. Пересоздаем command buffers для новых framebuffers
	uint32_t imageCount = m_SwapChainManager->GetImageCount ();
	m_CommandBuffers = m_CommandManager->CreateCommandBuffers ( imageCount );
	...
	LogDebug ( "Swapchain resources recreated successfully: ", imageCount, " command buffers" );
	return true;
	}
