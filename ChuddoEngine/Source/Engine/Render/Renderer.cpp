...
void CRenderer::RenderMeshes ( VkCommandBuffer CommandBuffer, uint32_t ImageIndex )
    ...
    void CRenderer::RenderTerrain ( VkCommandBuffer CommandBuffer, uint32_t ImageIndex )
        ...
        if (currentFrame >= m_FrameDescriptorSets.size ())
            ...
            // ПРОВЕРКА 2: Валидность CommandBuffer
        if (CommandBuffer == VK_NULL_HANDLE)
            {
        ...
        if (terrainPipeline == VK_NULL_HANDLE)
            {
            LogError ( "RenderTerrain: terrainPipeline is NULL" );
            return;
            }

        if (terrainLayout == VK_NULL_HANDLE)
            {
            LogError ( "RenderTerrain: terrainLayout is NULL" );
            return;
            }

        vkCmdBindPipeline ( CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, terrainPipeline );

        // ПРОВЕРКА 3: Bind global descriptor set ТОЛЬКО если он валиден
        ...
        if (globalSet != VK_NULL_HANDLE)
            ...
            {
            LOG_WARN ( "  No global descriptor set for terrain, using push constants only" );
            }
        ...
        // ПРОВЕРКА 4: Есть ли террейны для рендера
        if (terrains.empty ())
            {
        ...
        for (size_t i = 0; i < terrains.size (); i++)
            ...
            if (!terrain.IsValid ())
                ...
                // ПРОВЕРКА 5: Валидность вершинного буфера
