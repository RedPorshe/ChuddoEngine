...
#include "CoreMinimal.h"
    ...
    for (int i = 1; i < argc; ++i) 
    {
      ...
      if (arg.substr(0, 2) == "--")
      {
        ...
        if (equalsPos != std::string::npos)
        {
          ...
          CORE_DEBUG("Command line param: ", key, " = ", value);
        }
        else
        {
          std::string key = arg.substr(2);
          instance.m_Params[key] = "true";  
          CORE_DEBUG("Command line flag: ", key);
        }
      }
      else if (arg.substr(0, 1) == "-")
      {
        ...
        if (i + 1 < argc && argv[i + 1][0] != '-')
        {
          ...
          CORE_DEBUG("Command line param: -", key, " = ", argv[i + 1]);
          ++i;  
        }
        else
        {
      ...
      else
      {
        ...
        CORE_DEBUG("Command line argument: ", arg);
      }
    }

    CORE_LOG("Command line parsed: ", instance.m_Params.size(),
  ...
  int CommandLine::GetInt(const std::string& key, int defaultValue) const
  {
    ...
    if (it != m_Params.end())
    {
      ...
      {
        CORE_WARN("Invalid int value in command line for key: ", key);
      }
    }
    return defaultValue;
  }
  ...
  float CommandLine::GetFloat(const std::string& key, float defaultValue) const
  {
    ...
    if (it != m_Params.end())
    {
      ...
      {
        CORE_WARN("Invalid float value in command line for key: ", key);
      }
    }
    return defaultValue;
  }
