...
bool DeviceManager::Initialize(VkInstance instance, VkSurfaceKHR surface)
{
  ...
  {
    RENDER_ERROR("Failed to pick physical device");
    return false;
  }

  if (!CreateLogicalDevice(surface))
  {
    RENDER_ERROR("Failed to create logical device");
    return false;
  }

  RENDER_DEBUG("DeviceManager initialized successfully");
  return true;
}
...
void DeviceManager::Shutdown()
{
  if (m_device != VK_NULL_HANDLE)
  {
    ...
    RENDER_DEBUG("Logical device destroyed");
  }
...
bool DeviceManager::PickPhysicalDevice(VkInstance instance, VkSurfaceKHR surface)
{
  ...
  if (deviceCount == 0)
  {
    RENDER_ERROR("Failed to find GPUs with Vulkan support!");
    return false;
  }
  ...
  RENDER_DEBUG("Found ", deviceCount, " physical device(s)");
  ...
  if (candidates.rbegin()->first > 0)
  {
    ...
    RENDER_DEBUG("Selected physical device: ", deviceProperties.deviceName,
                 " (Score: ", candidates.rbegin()->first, ")");
    return true;
  }

  RENDER_ERROR("Failed to find a suitable GPU!");
  return false;
}
...
bool DeviceManager::CreateLogicalDevice(VkSurfaceKHR surface)
{
  ...
  {
    RENDER_ERROR("Failed to find required queue families!");
    return false;
  }
  ...
  RENDER_DEBUG("Logical device created successfully");
  RENDER_DEBUG("Graphics queue family: ", indices.graphicsFamily);
  RENDER_DEBUG("Present queue family: ", indices.presentFamily);
...
int DeviceManager::RateDeviceSuitability(VkPhysicalDevice device, VkSurfaceKHR surface)
{
  ...
  RENDER_DEBUG("Device ", deviceProperties.deviceName, " scored: ", score);
  return score;
}
...
bool DeviceManager::CheckDeviceExtensionSupport(VkPhysicalDevice device)
{
  ...
  {
    RENDER_DEBUG("Missing device extensions:");
    for (const auto& missing : requiredExtensions)
    {
      RENDER_DEBUG("  - ", missing);
    }
  }
